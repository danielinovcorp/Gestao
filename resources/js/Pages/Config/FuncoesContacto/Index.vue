...
function search() {
  router.get(
    route("config.funcoes-contacto.index"),
    { q: q.value },
...
function submit() {
  if (isEditing.value && editingId.value) {
    form.put(route("config.funcoes-contacto.update", editingId.value), {
      preserveScroll: true,
      onSuccess: () => (open.value = false),
    });
  } else {
    form.post(route("config.funcoes-contacto.store"), {
      preserveScroll: true,
      onSuccess: () => (open.value = false),
    });
  }
...
function destroyItem(id: number) {
  ...
  router.delete(route("config.funcoes-contacto.destroy", id), {
    preserveScroll: true,
  });
}
</script>
...
<template>
  <Head title="Configurações - Contactos (Funções)" />
  ...
  <AuthenticatedLayout>
    ...
    <div class="p-6 space-y-4">
      <div class="flex items-center gap-2">
        ...
        <Button @click="openCreate">Nova Função</Button>
      </div>
        ...
        <table class="min-w-full">
          <thead class="bg-slate-100">
            <tr>
              ...
              <th class="px-4 py-2 text-left text-sm font-semibold text-slate-700">Nome</th>
          ...
          <tbody class="bg-white divide-y divide-slate-200">
            ...
            >
              <td class="px-4 py-2 text-sm text-slate-600">{{ row.id }}</td>
              <td class="px-4 py-2 text-sm">{{ row.nome }}</td>
              <td class="px-4 py-2">
                <div class="flex gap-2 justify-end">
                  <Button size="sm" variant="secondary" @click="openEdit(row)">Editar</Button>
                  <Button size="sm" variant="destructive" @click="destroyItem(row.id)">Remover</Button>
                </div>
              </td>
            </tr>
      ...
      <div class="flex items-center justify-between text-sm text-slate-600">
        ...
        <div>Página {{ props.items.current_page }} de {{ props.items.last_page }}</div>
      </div>
    </div>
    ...
    <Dialog v-model:open="open">
      <DialogContent class="sm:max-w-[480px]">
        <DialogHeader>
          ...
          <DialogDescription>Formulário para criar ou editar uma função de contacto</DialogDescription>
        </DialogHeader>

        <!-- 👇 native form para submit estável -->
          ...
          <form @submit.prevent="submit" class="space-y-4">
            <FormField name="nome">
              <FormItem>
                ...
                <FormControl>
                  <Input v-model="form.nome" placeholder="Ex.: Diretor Comercial" />
                </FormControl>
                <FormMessage v-if="form.errors.nome">{{ form.errors.nome }}</FormMessage>
              </FormItem>
            </FormField>
            ...
            <div class="flex justify-end gap-2">
              ...
              <Button type="submit" :disabled="form.processing">
                {{ form.processing ? "A guardar..." : isEditing ? "Guardar" : "Criar" }}
              </Button>
            </div>
          </form>
        </UiForm>
