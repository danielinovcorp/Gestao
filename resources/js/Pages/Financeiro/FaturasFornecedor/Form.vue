<script setup lang="ts">
import { Head, Link, useForm } from "@inertiajs/vue3";
import AuthenticatedLayout from "@/Layouts/AuthenticatedLayout.vue";
import { ref, watch } from "vue";
import { Button } from "@/components/ui/button";
import {
    Form as UiForm,
    FormField,
    FormItem,
    FormLabel,
    FormControl,
    FormMessage,
} from "@/components/ui/form";
import { Input } from "@/components/ui/input";
import {
    Select,
    SelectTrigger,
    SelectValue,
    SelectContent,
    SelectItem,
} from "@/components/ui/select";
import { Textarea } from "@/components/ui/textarea";
import { format } from "date-fns";

const props = defineProps<{
    mode: "create" | "edit";
    fatura: any | null;
    fornecedores: { id: number; nome: string }[];
    encomendas: { id: number; numero: string }[];
}>();

const form = useForm({
    numero: props.fatura?.numero ?? "",
    data_fatura: props.fatura?.data_fatura ?? "",
    data_vencimento: props.fatura?.data_vencimento ?? "",
    fornecedor_id: props.fatura?.fornecedor_id ?? "",
    encomenda_fornecedor_id: props.fatura?.encomenda_fornecedor_id ?? "",
    valor_total: props.fatura?.valor_total ?? 0,
    estado: props.fatura?.estado ?? "pendente",
    documento: null as File | null,
    comprovativo: null as File | null,
});

const showComprovativoPrompt = ref(false);

watch(
    () => form.estado,
    (val, oldVal) => {
        if (oldVal !== "paga" && val === "paga") {
            showComprovativoPrompt.value = true;
        }
    },
);

function submit() {
    const routeName =
        props.mode === "create"
            ? "financeiro.faturas-fornecedor.store"
            : "financeiro.faturas-fornecedor.update";

    const options: any = {
        forceFormData: true,
        onSuccess: () => {},
    };

    if (props.mode === "create") {
        form.post(route(routeName), options);
    } else {
        form.post(route(routeName, props.fatura.id), {
            ...options,
            _method: "put",
        });
    }
}

function onFileChange(e: Event, key: "documento" | "comprovativo") {
    const t = e.target as HTMLInputElement;
    if (t.files && t.files[0]) form[key] = t.files[0];
}
</script>

<template>
    <Head :title="props.mode === 'create' ? 'Nova Fatura' : 'Editar Fatura'" />
    <AuthenticatedLayout>
        <div class="flex items-start justify-between mb-6">
            <h1 class="text-2xl font-semibold">
                {{
                    props.mode === "create"
                        ? "Nova Fatura de Fornecedor"
                        : "Editar Fatura de Fornecedor"
                }}
            </h1>
            <Link :href="route('financeiro.faturas-fornecedor.index')"
                ><Button variant="outline">Voltar</Button></Link
            >
        </div>

        <UiForm
            @submit.prevent="submit"
            class="grid grid-cols-1 md:grid-cols-2 gap-4"
        >
            <FormField name="numero">
                <FormItem>
                    <FormLabel>Número</FormLabel>
                    <FormControl>
                        <Input
                            v-model="form.numero"
                            placeholder="EX: FT-2025-001"
                        />
                    </FormControl>
                    <FormMessage v-if="form.errors.numero">{{
                        form.errors.numero
                    }}</FormMessage>
                </FormItem>
            </FormField>

            <FormField name="data_fatura">
                <FormItem>
                    <FormLabel>Data da Fatura</FormLabel>
                    <FormControl>
                        <Input type="date" v-model="form.data_fatura" />
                    </FormControl>
                    <FormMessage v-if="form.errors.data_fatura">{{
                        form.errors.data_fatura
                    }}</FormMessage>
                </FormItem>
            </FormField>

            <FormField name="data_vencimento">
                <FormItem>
                    <FormLabel>Data de Vencimento</FormLabel>
                    <FormControl>
                        <Input type="date" v-model="form.data_vencimento" />
                    </FormControl>
                    <FormMessage v-if="form.errors.data_vencimento">{{
                        form.errors.data_vencimento
                    }}</FormMessage>
                </FormItem>
            </FormField>

            <FormField name="fornecedor_id">
                <FormItem>
                    <FormLabel>Fornecedor</FormLabel>
                    <FormControl>
                        <Select v-model="form.fornecedor_id">
                            <SelectTrigger
                                ><SelectValue
                                    placeholder="Selecionar fornecedor"
                            /></SelectTrigger>
                            <SelectContent>
                                <SelectItem
                                    v-for="f in fornecedores"
                                    :key="f.id"
                                    :value="String(f.id)"
                                    >{{ f.nome }}</SelectItem
                                >
                            </SelectContent>
                        </Select>
                    </FormControl>
                    <FormMessage v-if="form.errors.fornecedor_id">{{
                        form.errors.fornecedor_id
                    }}</FormMessage>
                </FormItem>
            </FormField>

            <FormField name="encomenda_fornecedor_id">
                <FormItem>
                    <FormLabel>Encomenda Fornecedor</FormLabel>
                    <FormControl>
                        <Select v-model="form.encomenda_fornecedor_id">
                            <SelectTrigger
                                ><SelectValue placeholder="(Opcional)"
                            /></SelectTrigger>
                            <SelectContent>
                                <SelectItem
                                    v-for="e in encomendas"
                                    :key="e.id"
                                    :value="String(e.id)"
                                    >{{ e.numero }}</SelectItem
                                >
                            </SelectContent>
                        </Select>
                    </FormControl>
                    <FormMessage v-if="form.errors.encomenda_fornecedor_id">{{
                        form.errors.encomenda_fornecedor_id
                    }}</FormMessage>
                </FormItem>
            </FormField>

            <FormField name="valor_total">
                <FormItem>
                    <FormLabel>Valor Total</FormLabel>
                    <FormControl>
                        <Input
                            type="number"
                            step="0.01"
                            min="0"
                            v-model="form.valor_total"
                        />
                    </FormControl>
                    <FormMessage v-if="form.errors.valor_total">{{
                        form.errors.valor_total
                    }}</FormMessage>
                </FormItem>
            </FormField>

            <FormField name="documento">
                <FormItem>
                    <FormLabel>Documento (anexo)</FormLabel>
                    <FormControl>
                        <Input
                            type="file"
                            @change="(e) => onFileChange(e, 'documento')"
                        />
                    </FormControl>
                    <div v-if="props.fatura?.documento_url" class="text-sm">
                        <a
                            :href="props.fatura.documento_url"
                            class="text-indigo-600 hover:underline"
                            target="_blank"
                            >Ver atual</a
                        >
                    </div>
                    <FormMessage v-if="form.errors.documento">{{
                        form.errors.documento
                    }}</FormMessage>
                </FormItem>
            </FormField>

            <FormField name="estado">
                <FormItem>
                    <FormLabel>Estado</FormLabel>
                    <FormControl>
                        <Select v-model="form.estado">
                            <SelectTrigger
                                ><SelectValue placeholder="Selecionar estado"
                            /></SelectTrigger>
                            <SelectContent>
                                <SelectItem value="pendente"
                                    >Pendente de Pagamento</SelectItem
                                >
                                <SelectItem value="paga">Paga</SelectItem>
                            </SelectContent>
                        </Select>
                    </FormControl>
                    <FormMessage v-if="form.errors.estado">{{
                        form.errors.estado
                    }}</FormMessage>
                </FormItem>
            </FormField>

            <!-- Prompt para comprovativo quando marcar "Paga" -->
            <div
                v-if="showComprovativoPrompt"
                class="md:col-span-2 rounded-lg border p-4 bg-yellow-50"
            >
                <div class="font-medium mb-2">
                    Pretende enviar o comprovativo ao Fornecedor?
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div>
                        <label class="text-sm block mb-1"
                            >Anexar Comprovativo</label
                        >
                        <Input
                            type="file"
                            @change="(e) => onFileChange(e, 'comprovativo')"
                        />
                    </div>
                    <div class="self-end text-sm text-gray-600">
                        O email será enviado automaticamente ao gravar.
                    </div>
                </div>
            </div>

            <div class="md:col-span-2 flex justify-end gap-2 mt-4">
                <Link :href="route('financeiro.faturas-fornecedor.index')"
                    ><Button variant="outline">Cancelar</Button></Link
                >
                <Button type="submit">{{
                    props.mode === "create" ? "Criar" : "Guardar"
                }}</Button>
            </div>
        </UiForm>
    </AuthenticatedLayout>
</template>
